...
internals.setVersion = function (path, version) {
    ...
    internals.versions[path.replace(/{[^}]*}/, '.*')] = version;
}

internals.versionMatch = function (path) {

    for (const versionPath in internals.versions) {
        const version = internals.versions[versionPath];
...
internals.route = function (request, reply) {
    ...
    const v = internals.versionMatch(request.path);
    if (!v) {
